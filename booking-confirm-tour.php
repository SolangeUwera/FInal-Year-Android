...
if (empty($_SESSION['currency']) || $_SESSION['currency'] == "USD") {
	...
	$_SESSION['c_to'] = "USD";
	$_SESSION['c_symbol'] = "$";
} else if ($_SESSION['currency'] == "RW") {
	$_SESSION['c_from'] = "USD";
	$_SESSION['c_to'] = "RW";
	$_SESSION['c_symbol'] = "RW";
}
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	$to = "rigorene48@gmail.com"; // this is your Email address
	$from = $enq_tour_email; // this is the sender's Email address
	$subject = "Confirmation Booking ";
	$subject2 = "Response from TravelApp.com for Tour Booking";
	...
	$statement = "insert into tour_enquiry
  (tour_id, name, email, phone, days, child, adult, message,status,payment_method,addedBy) 
  values ('$_SESSION[tour_id]', '$enq_tour_name', '$enq_tour_email',
   '$enq_tour_phone', '$enq_tour_days', '$enq_tour_child', 
   '$enq_tour_adult', '$enq_tour_message', 'confirmed','$payment_Method' ,'$addedBy')";

	$seat_available_query = mysqli_query($conn, "SELECT available 
FROM tours WHERE tour_id= '$_SESSION[tour_id]' and deletedAt is NULL ");
	...
	if (mysqli_query($conn, $statement)) {
		...
		$message = 'Hello '.$enq_tour_name.', you booked '.$tour_title. 'with '.($enq_tour_child + $enq_tour_adult).'seats numbers and amount to pay is  ' .  $_SESSION['c_symbol'].  $price.' per each seat and Total Amount is'.
		     ($_SESSION['c_symbol'] . ($enq_tour_child + $enq_tour_adult) * $price);
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            CURLOPT_POSTFIELDS => array(
                'action' => 'send-sms',
                'to' =>'+25'. $enq_tour_phone, 'from' => 'Park Book', 'sms'
                => $message, 'api_key' => 'aThjNHhxN3h5bWl4Mmtld3E2djE1Y3R1c2ZodDMzOWw='
            ),
        ));
											...
											<div class="row">
												<form role="form" name="hotels-enq" method="post" action="">
													...
													<table class="table table-hover">
														...
														<tbody>
															<tr>
																<td class="col-md-9"><em><?php echo $tour_title ?></em></h4>
																</td>
															...
															<tr>
																...
																<td class="text-center text-danger">
																	<h4><strong><?php echo $_SESSION['c_symbol'] . ($enq_tour_child + $enq_tour_adult) * $price; ?></strong></h4>
																</td>
															</tr>
														</tbody>
													</table>
