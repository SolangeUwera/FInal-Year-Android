...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	if ($$seat_available['available'] === NULL) {
		$cap_available_query = mysqli_query($conn, "SELECT capacity FROM tours WHERE tour_id= '$_SESSION[tour_id]' ");
		$seat_available_new = mysqli_fetch_assoc($cap_available_query);
		$available = $seat_available_new['capacity'];
	} else {
	...
	if (($_SESSION['enq_tour_child'] + $_SESSION['enq_tour_adult']) <= $available) {
		...
		var_dump($seat_available);
		$notifyMsg = "Sorry. No Rooms available we only have this available place $available";
	}
