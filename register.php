...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	$role =  data_sanitization($_POST['role']);


	$duplicate_check = mysqli_query($conn, "SELECT username from users where username='$username'");
	$duplicate_count = mysqli_num_rows($duplicate_check);


	$duplicate_check_email = mysqli_query($conn, "SELECT email from users where email='$email'");
											...
											<div class="room-wrap">
												<div class="row">
													<form role="form" name="hotels-enq" method="post" action="" enctype="multipart/form-data">
														...
														<div class="form-group">
															<label>Phone</label>
															<input type="number" class="form-control" id="sub-input"  placeholder="Your Phone Number" name="phone" value="" required pattern="^[0-9]{3,15}$" title="Enter Your Phone Number">
														</div>
														<div class="form-group">
														...
														<button type="submit" name="register_user" class="btn btn-primary">SUBMIT</button>
													</form>
												</div>
											</div>
										</div>
