...
$room_available_query=mysqli_query($conn, "SELECT * FROM room_type 
WHERE hotel_id= '$_SESSION[hotel_id]' and room_type_id= '$_SESSION[room_type_id]' ");
$room_available= mysqli_fetch_assoc($room_available_query);
$available= $room_available['available'];
$favailable=$room_available['available'];
$capacity=$room_available['capacity'];

$room_available_query_enq=mysqli_query($conn, "SELECT * FROM hotel_enquiry
 WHERE hotel_id= '$_SESSION[hotel_id]' and room_type_id= '$_SESSION[room_type_id]' ");
$room_available_enq= mysqli_fetch_assoc($room_available_query_enq);
$checkout= $room_available_enq['checkout'];

if($_SESSION['enq_hotel_room'] <= $available && $available>0
 ||$available<=0 && $_SESSION['enq_hotel_room'] <= $favailable && $capacity<=$favailable    )
{
